...
export class SerpGoogleMapsTasksReadyTaskInfo  extends BaseResponseTaskInfo   implements ISerpGoogleMapsTasksReadyTaskInfo {
    ...
    constructor(data?: ISerpGoogleMapsTasksReadyTaskInfo) {
    super(data);
        ...
        if (data) {
            ...
            if (Array.isArray(data["result"])) {
                ...
                for (let item of data["result"]) {
                    this.result.push(SerpGoogleMapsTasksReadyResultInfo.fromJS(item));
                }
            }
        }
    }

    static fromJS(data: any): SerpGoogleMapsTasksReadyTaskInfo {
        data = typeof data === 'object' ? data : {};


        let result = new SerpGoogleMapsTasksReadyTaskInfo();
        result.init(data);
        return result;
    }
