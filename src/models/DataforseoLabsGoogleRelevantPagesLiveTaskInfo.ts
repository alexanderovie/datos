...
export class DataforseoLabsGoogleRelevantPagesLiveTaskInfo  extends BaseResponseTaskInfo   implements IDataforseoLabsGoogleRelevantPagesLiveTaskInfo {
    ...
    constructor(data?: IDataforseoLabsGoogleRelevantPagesLiveTaskInfo) {
    super(data);
        ...
        if (data) {
            ...
            if (Array.isArray(data["result"])) {
                ...
                for (let item of data["result"]) {
                    this.result.push(DataforseoLabsGoogleRelevantPagesLiveResultInfo.fromJS(item));
                }
            }
        }
    }

    static fromJS(data: any): DataforseoLabsGoogleRelevantPagesLiveTaskInfo {
        data = typeof data === 'object' ? data : {};


        let result = new DataforseoLabsGoogleRelevantPagesLiveTaskInfo();
        result.init(data);
        return result;
    }
