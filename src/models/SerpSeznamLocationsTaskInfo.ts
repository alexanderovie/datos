...
export class SerpSeznamLocationsTaskInfo  extends BaseResponseTaskInfo   implements ISerpSeznamLocationsTaskInfo {
    ...
    constructor(data?: ISerpSeznamLocationsTaskInfo) {
    super(data);
        ...
        if (data) {
            ...
            if (Array.isArray(data["result"])) {
                ...
                for (let item of data["result"]) {
                    this.result.push(SerpSeznamLocationsResultInfo.fromJS(item));
                }
            }
        }
    }

    static fromJS(data: any): SerpSeznamLocationsTaskInfo {
        data = typeof data === 'object' ? data : {};


        let result = new SerpSeznamLocationsTaskInfo();
        result.init(data);
        return result;
    }
