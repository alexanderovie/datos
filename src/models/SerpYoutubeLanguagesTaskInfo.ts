...
export class SerpYoutubeLanguagesTaskInfo  extends BaseResponseTaskInfo   implements ISerpYoutubeLanguagesTaskInfo {
    ...
    constructor(data?: ISerpYoutubeLanguagesTaskInfo) {
    super(data);
        ...
        if (data) {
            ...
            if (Array.isArray(data["result"])) {
                ...
                for (let item of data["result"]) {
                    this.result.push(SerpYoutubeLanguagesResultInfo.fromJS(item));
                }
            }
        }
    }

    static fromJS(data: any): SerpYoutubeLanguagesTaskInfo {
        data = typeof data === 'object' ? data : {};


        let result = new SerpYoutubeLanguagesTaskInfo();
        result.init(data);
        return result;
    }
