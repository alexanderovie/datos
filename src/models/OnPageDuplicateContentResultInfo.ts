...
possible values: in_progress, finished */
    ...
    constructor(data?: IOnPageDuplicateContentResultInfo) {
        ...
        if (data) {
            ...
            if (Array.isArray(data["items"])) {
                ...
                for (let item of data["items"]) {
                    this.items.push(OnPageDuplicateContentItem.fromJS(item));
                }
            }
        }
    }

    static fromJS(data: any): OnPageDuplicateContentResultInfo {
        data = typeof data === 'object' ? data : {};


        let result = new OnPageDuplicateContentResultInfo();
        result.init(data);
        return result;
    }
